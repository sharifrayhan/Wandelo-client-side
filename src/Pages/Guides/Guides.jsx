            ...
            <div className=" items-center justify-center mt-10 flex flex-wrap gap-4">
                ...
                {filteredGuides &&
                    filteredGuides?.map((guide) => (
                        <div key={guide._id} className="bg-[#f7f5f2] text-center p-6 rounded-lg shadow-lg">
                            <img src={guide.profile_image} alt={guide.name} className="mb-4 h-32 w-full object-cover rounded-md" />
                            <span className="text-gray-400">Joined: {guide?.createdAt?.substring(0, 10)}</span>
                            ...
                            <div className="flex justify-center items-center">
                                <Link to={`/GuideDetails/${guide._id}`}><button className="bg-[#e1a66f] hover:bg-slate-400 text-white px-4 py-2 rounded-md">Details</button></Link>
